...
function Button({
  ...
  const variants = {
    default: "bg-white/[0.06] text-white/90 hover:bg-white/[0.1] border border-white/[0.08]",
    primary: "bg-blue-600 text-white hover:bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.3)] border border-blue-500/50",
  ...
  return (
    <button
      className={cn(
        "inline-flex items-center justify-center gap-2 rounded-lg text-sm font-medium transition-all duration-200 active:scale-95 disabled:opacity-50",
        variants[variant],
        sizes[size],
        className
      )}
...
function Card({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        "rounded-xl border border-white/[0.08] bg-[#0a0a0a]/80 backdrop-blur-xl shadow-lg",
        className
      )}
    >
      {children}
            ...
            <Card className="overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="neutral">All Status</Badge>
                    ...
                    {tableData.map((row) => (
                      <tr key={row.id} className="group hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className={cn(
                          ...
                          row.growth.startsWith("+") ? "text-emerald-400" : "text-red-400"
                        )}>
                          {row.growth}
                        </td>
