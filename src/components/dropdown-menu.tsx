...
function DropdownMenuItem({
	...
	return (
		<button
			...
			onClick={handleClick}
			className={cn(
				"flex w-full items-center rounded-md px-2 py-1.5 text-sm text-zinc-700 dark:text-zinc-300 outline-none transition-colors hover:bg-black/5 dark:hover:bg-white/10 hover:text-zinc-900 dark:hover:text-zinc-100 focus:bg-black/5 dark:focus:bg-white/10",
...
function DropdownMenuShortcut({ children, className }: DropdownMenuShortcutProps) {
	return (
		<span className={cn("ml-auto text-xs tracking-widest text-zinc-500 dark:text-zinc-400", className)}>
			{children}
		</span>
	);
}
