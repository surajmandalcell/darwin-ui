...
export function Table({ children }: { children: React.ReactNode }) {
	return (
		<div className="w-full border border-white/10 rounded-md overflow-x-auto md:overflow-visible">
			<table className="w-full text-sm text-white/90">{children}</table>
		</div>
	);
}
...
export function TableHeaderCell({
	...
	return (
		<th
			scope="col"
			className={`px-4 py-2 text-left align-middle font-medium text-white/80 ${className}`}
			{...props}
		>
			{children}
		</th>
...
export function TableRow({
	...
	return (
		<tr
			className={`border-b border-white/10 transition-opacity duration-300 ${className}`}
